...
export default async (_req: NextApiRequest, res: NextApiResponse) => {
  const data = await GetChartsData(1000, new Date(`${year}-${month}-15`));
  ...
  subviewSection(15, 70, 12.5, 85, () => {
    ...
    svg(
      ...
      barChart({
        ...
        domain: [0, 100],
        bar: barStacked({ fills: [cyan, yellow, "white"] }),
      }),
    ...
    svg(
      ...
      barChart({
        ...
        /*sections: data.weatherData.map((entry, i) => ({
          name: entry.name.toString(),
          data: [20 + i, 10, 20],
        })),*/
        marginTop: rvh(2),
    ...
    svg(
      ...
      barChart({
        ...
        domain: [
          0,
          Math.max(...data.workersData.map((entry) => entry.data.reduce((a, b) => a + b))),
        ],
    ...
    text(doc, `${Object.values<number>(data.workersAverage).reduce((a, b) => a + b)} فرد \\ يوم`, {
      x0: vw(70),
      x1: vw(100),
      y0: vh(53),
      weight: "bold",
    ...
    text(
      doc,
      `${(
        data.weatherData.map((entry) => entry.data[1]).reduce((a, b) => a + b) /
        data.weatherData.length
      ).toFixed(1)} \\ ${(
        data.weatherData.map((entry) => entry.data[0]).reduce((a, b) => a + b) /
        data.weatherData.length
      ).toFixed(1)}`,
      {
        x0: vw(70),
