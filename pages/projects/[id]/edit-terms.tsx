...
import { TermsAttributes } from '../../../db/types';
...
const Terms = () => {
  ...
  const {
    ...
    getProjects, fetchProjectData, updateBulkData
  } = useProjects();
  ...
  useEffect(() => {
    if(terms.length > 0){
      setData([...terms.filter((t: TermsAttributes) => t.projectId === +id).map(
        (t: TermsAttributes) => ([
          {value: t.name, name: "name", id: t.id, type: "string", readOnly: false, edited: false},
          {value: t.percentage, name: "percentage", id: t.id, type: "number", readOnly: false, edited: false},
          {value: t.value, name: "value", id: t.id, type: "number", readOnly: false, edited: false}
        ])
      )])
  ...
  const save = async () => {
    const bulkData : any[] = [];
    data.forEach((r: any) => {
      const obj : any = {id: r[0].id, projectId: +id};
      let notEmpty = true;
      let edited = false;
      console.log(r)
      r.map((c: any) => {
    ...
    //const { startDate, currentExpirationDate } =
    //projects.find((p: ProjectsAttributes) => p.id === +id)!
    if(bulkData.length){
      await updateBulkData("terms", +id, bulkData, []);
    }
  }
        ...
        <div className="row no-gutters">
          <div className="col-12">
            ...
            <div className="d-flex justify-content-end mt-3 mb-1">
              ...
              <button className="btn btn-warning mx-2 mb-3 w-25" onClick={goBack}>
