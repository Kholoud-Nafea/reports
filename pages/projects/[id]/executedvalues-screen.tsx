...
const ExecutedValues = () => {
  ...
  useEffect(() => {
    ...
    if(diaries.length > 0){
      ...
      setData((_ps: any) => filtered.map(
        (d: DiariesAttributes) => ([
          {value: d.businessValue, name: "businessValue", type: "number", id: d.id, edited: false}
        ])
      ))
    }
  ...
  const save = async () => {
    ...
    //console.log(arr);
    if(arr.length){
      await updateBulkData("diaries", +id, arr, ["businessValue"]);
    }
  }
