...
const Status = () => {
  ...
  useEffect(() => {
    ...
    if(diaries.length > 0){
      ...
      setData((_ps: any) => filtered.map(
        (d: DiariesAttributes) => ([
          {
            value: d.updatingStatus, name: "updatingStatus", type: "select",
            options:["منتظم", "متأخر","لم يقدم"],id: d.id, edited: false
          }
        ])
      ))
    }
  ...
  const save = async () => {
    ...
    //console.log(arr);
    if(arr.length){
      await updateBulkData("diaries", +id, arr, ["updatingStatus"]);
    }
